...
=======
    ...
    def deposit(self, amount, currency_code):
        ...
        transaction = Transaction.objects.create(
            ...
            fee=0  # You can implement fee calculation if applicable
        )
    ...
    def withdraw(self, amount, currency_code):
       if currency_code == 'NIS':
        ...
        try:
            ...
            raise ValueError(f"Currency {currency_code} not supported.")
       transaction = Transaction.objects.create(
            account=self,  # Link the transaction to this bank account
            transaction_type='withdraw',  # Specify the type of transaction
            amount=Decimal(amount) if currency_code == 'NIS' else amount_in_nis,  # Set the amount
            fee=5.0 # You can implement fee calculation if applicable
        )
    ...
    def transfer(self, amount, recipient_account, currency_code='NIS'):
        ...
        transaction = Transaction.objects.create(
            account=self,  # Link the transaction to this bank account
            transaction_type='transfer',  # Specify the type of transaction
            amount=Decimal(amount) if currency_code == 'NIS' else amount_in_nis,  # Set the amount
            fee=8.0 # You can implement fee calculation if applicable
        )
